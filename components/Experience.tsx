...
const Card: React.FC<{ icon: React.ReactNode; title: string; subtitle: string; description?: string; year?: string }> = ({ icon, title, subtitle, description, year }) => (
  <div className="bg-white group p-6 md:p-8 rounded-2xl border border-zinc-100 hover:border-red-100 hover:shadow-[0_8px_30px_rgb(239,68,68,0.06)] transition-all duration-300 h-full flex flex-col relative overflow-hidden">
    ...
    <div className="flex justify-between items-start mb-6">
        <div className="w-12 h-12 bg-red-50 text-red-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shrink-0">
            {icon}
        </div>
        {year && <span className="text-xs font-bold bg-zinc-100 text-zinc-500 px-3 py-1 rounded-full group-hover:bg-red-600 group-hover:text-white transition-colors whitespace-nowrap ml-2">{year}</span>}
    </div>
    
    <h3 className="text-xl font-bold text-zinc-900 mb-1 group-hover:text-red-600 transition-colors">{title}</h3>
    <p className="text-zinc-400 text-sm font-medium mb-4 uppercase tracking-wide">{subtitle}</p>
    {description && <p className="text-zinc-500 text-sm leading-relaxed mt-auto">{description}</p>}
  </div>
);

const Experience: React.FC = () => {
  return (
    <section className="relative w-full bg-zinc-50/50 py-24 md:py-32 px-6 md:px-12 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-20 gap-6">
             ...
             <p className="text-zinc-500 max-w-md text-left md:text-right text-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
             ...
             <div className="flex flex-col gap-6">
                <h3 className="text-2xl font-bold text-zinc-900 flex items-center gap-3 mb-4">
             ...
             <div className="flex flex-col gap-6">
                <h3 className="text-2xl font-bold text-zinc-900 flex items-center gap-3 mb-4">
                    <span className="w-8 h-1 bg-zinc-300 rounded-full"></span> Career
                </h3>
                ...
                <div className="bg-zinc-900 rounded-2xl p-8 text-white min-h-[220px] flex flex-col justify-center relative overflow-hidden group">
